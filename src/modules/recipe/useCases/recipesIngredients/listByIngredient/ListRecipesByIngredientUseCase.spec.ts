...
let listRecipesByIngredientUseCase: ListRecipesByIngredientUseCase;
...
describe("List recipes by ingredient UseCase", () => {
    beforeEach(() => {
        ...
        listRecipesByIngredientUseCase = new ListRecipesByIngredientUseCase(
            recipeRepositoryInMemory,
            ingredientRepositoryInMemory
        );
    });
    ...
    it("should be avle to list recipes by ingredient", async () => {
        ...
        const recipes = await listRecipesByIngredientUseCase.execute(
            createIngredient1.id
        );
